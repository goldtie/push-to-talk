...
public class MessageFactory extends org.zoolu.sip.message.BaseMessageFactory {
	...
	public static Message createPublishRequest(SipProvider sip_provider,
			SipURL recipient, NameAddress to, NameAddress from,
			NameAddress contact, String event, String id, String content_type,
			String body) {
		String allow = null;
		Message req = createRequest(sip_provider, SipMethods.PUBLISH,
				recipient, to, from, contact, allow, null);
		req.setEventHeader(new EventHeader(event, id));
		req.setBody(content_type, body);
		return req;
	}

	/** Creates a new PUBLISH request within a dialog (re-publish). */
	public static Message createPublishRequest(Dialog dialog, String event,
			String id, String content_type, String body) {
		Message req = createRequest(dialog, SipMethods.PUBLISH, null);
		req.setEventHeader(new EventHeader(event, id));
		req.setBody(content_type, body);
		return req;
	}
