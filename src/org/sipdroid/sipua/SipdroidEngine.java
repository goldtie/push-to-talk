...
public class SipdroidEngine implements RegisterAgentListener {
	...
	public void halt() { // modified
		Log.d("HAO", "sipdroidEngine_Class-->halt");
		...
		for (RegisterAgent ra : ras) {
			...
			if (ra != null)
				ra.halt();
			if (uas[i] != null)
				Log.d("HAO", "sipdroidEngines-->halt()==>HangUP");
			uas[i].hangup();
	...
	public void onUaRegistrationFailure(RegisterAgent reg_ra,
		...
		if (isRegistered(i)) {
			...
			Receiver.onText(Receiver.REGISTER_NOTIFICATION + i, getUIContext()
					...
					+ " (" + result + ")", R.drawable.sym_presence_away, 0);
		}
	...
	public void updateDNS() {
		...
		for (SipProvider sip_provider : sip_providers) {
			try {
				edit.putString(Settings.PREF_DNS + i, IpAddress.getByName(
	...
	public boolean call(String target_url, boolean force) {
		...
		if (!found || (ua = uas[p]) == null) {
			if (PreferenceManager.getDefaultSharedPreferences(getUIContext())
					...
					&& PreferenceManager.getDefaultSharedPreferences(
							getUIContext()).getString(Settings.PREF_POSURL,
	...
	public void rejectcall() {
		Log.d("HAO", "sipdroidEngines-->rejectcall()==>HangUP");
	...
	public int speaker(int mode) {
		android.util.Log.d("SIPDROID", "[SipdroidEngine] - speaker - mode = "
	...
	public boolean isFire() {
		if (user_profiles[0].username.equals("fire"))
			return true;
		else if (user_profiles[1].username.equals("fire"))
			return true;
		return false;
	}
}
